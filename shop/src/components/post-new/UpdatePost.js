...
const UpdatePost = ({item, onSubmit, forceRender}) => {
  ...
  return (
    <SWrapperForm>
      ...
      <Box component="form" style={{display: 'flex', flexDirection: 'column'}}>
        <SCustomTextInput value={name}  onChange={onChangeText} sx={{
          marginBottom: '5px'
        }} name="name" label="name" variant="outlined" /> 
        <TextareaAutosize  value={description}  onChange={onChangeText} style={{
          height: 200
        ...
        <Button onClick={onClickButton} variant="contained">Change</Button>
      </Box>
    </SWrapperForm>
  )
}
